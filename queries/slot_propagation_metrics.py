...
def fetch_raw_aggregations(
    ...
    SELECT
        a.slot,
        a.message_id,
        a.slot_start_date_time as slot_start_time,
        a.event_date_time as agg_arrival,
        a.propagation_slot_start_diff as agg_arrival_wb,
        a.meta_client_name as local_node,
        a.peer_id_unique_key as remote_node
    FROM libp2p_gossipsub_aggregate_and_proof a
    PREWHERE {date_filter}
    WHERE meta_network_name = '{network}' and agg_arrival_wb >= 7
    ORDER BY slot, committee_index, aggregator_index, message_id DESC
    """
        dfs.append(client.query_df(query))
    return pd.concat(dfs), query
...
def fetch_iwants(
    ...
    hour_interval = 1
    for base_h in range(0, 24, hour_interval):
        date_filter_a = _manual_date_filter(
            target_date,
            base_h=14,
            h_interval=1,
        )
        date_filter_b = _manual_date_filter(
            target_date,
            base_h=14,
            h_interval=1,
...
WITH
    ...
    iwants as (
        SELECT
            iwants.message_id,
            iwants.event_date_time,
            iwants.meta_client_name as local_node,
            iwants.peer_id_unique_key remote_node,
            p.client
        FROM libp2p_rpc_meta_control_iwant iwants
        GLOBAL INNER JOIN peers p ON iwants.peer_id_unique_key = p.peer_id
        PREWHERE {date_filter_b}
        WHERE meta_network_name = '{network}'
    ),
    aggregations as (
        SELECT
            ...
            a.slot_start_date_time as slot_start_time,
            a.event_date_time as agg_arrival,
            a.event_date_time -slot_start_date_time as agg_arrival_wb,
            a.meta_client_name as local_node,
            a.peer_id_unique_key as remote_node
        FROM libp2p_gossipsub_aggregate_and_proof a
        PREWHERE {date_filter_a}
        WHERE meta_network_name = '{network}' and agg_arrival_wb >= 7
        ORDER BY slot, committee_index, aggregator_index, message_id DESC
    )
SELECT
    a.slot,
    i.message_id,
    i.event_date_time as iwant_date_time,
    (i.event_date_time - a.slot_start_time) as iwant_wb,
    i.local_node,
    i.remote_node,
    i.client,
    a.agg_arrival,
    ...
    a.slot_start_time
FROM iwants i
GLOBAL INNER JOIN aggregations a ON
    i.message_id = a.message_id
    and i.local_node = a.local_node
    and i.remote_node = i.remote_node
ORDER BY slot
        """
        dfs.append(client.query_df(query))
    return pd.concat(dfs), query