...
export default defineChart({
  id: 'blocks-by-blobcount-mev-local',
  topic: 'mev-pipeline',
  title: 'Block arrival latency: MEV vs local',
  description: 'Horizontal grouped boxplots of block arrival latency split by MEV vs local builder, per blob count.',
  queries: ['block_events'] as const,
  related: ['relay-box-by-blobbin', 'first-col-by-blob-mev-local'],
  context: () =>
    import(
      '../context/mev_pipeline/blocks_by_blobcount_mev_local.md?raw'
  ...
  option(data) {
    ...
    return {
      ...
      xAxis: { type: 'value', name: 'block first-seen (ms)' },
