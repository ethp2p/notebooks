...
export default defineChart({
  id: 'first-col-by-blob-mev-local',
  topic: 'mev-pipeline',
  title: 'First column seen: MEV vs local',
  description: 'Horizontal grouped boxplots of first column seen latency split by MEV vs local builder, per blob count.',
  queries: ['block_events'] as const,
  related: ['last-col-by-blob-mev-local', 'blocks-by-blobcount-mev-local'],
  context: () =>
    import(
      '../context/mev_pipeline/first_col_by_blob_mev_local.md?raw'
  ...
  transform(raw) {
    ...
    const slotFirstCol = new Map<number, number>();
    ...
    for (let i = 0; i < t.numRows; i++) {
      ...
      if (evType === 'first_column_seen') {
        slotFirstCol.set(s, lat);
        slotBlobs.set(s, Number(blobsCol?.get(i) ?? 0));
        slotIsMev.set(s, Boolean(isMevCol?.get(i)));
      }
    }

    for (const [s, lat] of slotFirstCol) {
  ...
  option(data) {
    ...
    return {
      ...
      xAxis: { type: 'value', name: 'first column seen (ms)' },
