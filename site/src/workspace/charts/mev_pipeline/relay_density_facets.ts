...
export default defineChart({
  id: 'relay-density-facets',
  topic: 'mev-pipeline',
  title: 'Relay density facets',
  description: '2D density heatmap of winning bid vs block arrival latency, one grid per relay (up to 9).',
  queries: ['block_events'] as const,
  related: ['builder-density-facets', 'bid-vs-block-scatter'],
  context: () =>
    import(
      '../context/mev_pipeline/relay_density_facets.md?raw'
  ...
  transform(raw) {
    ...
    for (let i = 0; i < t.numRows; i++) {
      ...
      if (evType === 'bid_winning') {
        winningMs.set(s, lat);
        relayBySlot.set(s, String(relayCol?.get(i) ?? 'unknown'));
      } else if (evType === 'block_arrival') {
        arrivalMs.set(s, lat);
      }
    }

    type Point = { relay: string; x: number; y: number };
    const points: Point[] = [];
    ...
    for (const [s, w] of winningMs) {
      ...
      if (a == null) continue;
      const relay = relayBySlot.get(s) ?? 'unknown';
      points.push({ relay, x: w, y: a });
      relayCounts.set(relay, (relayCounts.get(relay) ?? 0) + 1);
    }

    const topRelays = [...relayCounts.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, MAX_FACETS)
      .map(([r]) => r);

    if (topRelays.length === 0) return { facets: [], xLabels: [], yLabels: [] };
    ...
    const topSet = new Set(topRelays);
    const facets = topRelays.map((relay) => {
      const grid = new Map<number, number>();
      for (const p of points) {
        if (!topSet.has(p.relay) || p.relay !== relay) continue;
      ...
      return { label: relay, cells, maxCount };
    });
  ...
  option(data) {
    ...
    return {
      ...
      visualMap: {
        ...
        min: 0,
        max: globalMax,
        inRange: { color: [t.bg1, t.accent.amber] },
        calculable: false,
        show: false,
      },
      series,
