...
export default defineChart({
  id: 'relay-blobcount-sankey',
  topic: 'blob-flow',
  title: 'Relay to blob count',
  description: 'Block counts per (relay, blob count) pair.',
  queries: ['blob_flow_edges'] as const,
  related: [],
  context: () =>
    import(
      '../context/blob_flow/relay_blobcount.md?raw'
  ...
  transform(raw) {
    ...
    for (let i = 0; i < table.numRows; i++) {
      if (String(stage[i]) !== 'relay_to_blob') continue;
